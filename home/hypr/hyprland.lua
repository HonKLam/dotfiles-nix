...
hl.config({
    general = {
        ...
        resize_on_border = true, -- FIX: was false in your draft, true in your working .conf
...
-- NOTE: windows/windowsIn/windowsOut restored to bezier="easeOutQuint" to match your working .conf.
-- Your draft had switched these to spring="easy", which changes the animation feel — revert if that was on purpose.
...
-- (not active since general.layout = "master", left in as template default — harmless)
hl.config({
    dwindle = {
        preserve_split = true,
    },
...
hl.config({
    master = {
        new_status = "slave", -- FIX: was "master" in your draft, "slave" in your working .conf
	mfact = 0.5,
	smart_resizing = true,
...
hl.config({
    misc = {
        force_default_wallpaper   = -1,   -- harmless template default (unused since disable_hyprland_logo below suppresses the mascot anyway)
        disable_hyprland_logo     = true,
        disable_splash_rendering  = true, -- restored, was missing
    },
})
...
hl.config({
    input = {
        ...
        sensitivity = -0.3, -- FIX: was 0 in your draft, -0.3 in your working .conf
...
-- SUPER+SHIFT+M restored to addmaster (see master-layout binds below).
-- Shutdown command moved to SUPER+ALT+SHIFT+M so it isn't lost.
hl.bind(mainMod .. " + ALT + SHIFT + M", hl.dsp.exec_cmd("command -v hyprshutdown >/dev/null 2>&1 && hyprshutdown || hyprctl dispatch 'hl.dsp.exit()'"))
...
-- Move focus with mainMod + HJKL (restored, was missing from your draft)
...
-- Resize with keys (restored, was missing)
-- Confirmed shape from wiki example: hl.dsp.window.resize({ x = 10, y = 0, relative = true })
-- relative = true matches the old resizeactive behavior (nudge by N px, not set absolute size)
...
-- Set focused window as Master
hl.bind(mainMod .. " + M", hl.dsp.layout("swapwithmaster"))

-- Add focused window as additional Master
hl.bind(mainMod .. " + SHIFT + M", hl.dsp.layout("addmaster"))

-- Remove focused window as Master
...
-- Screenshots via hyprshot (restored, was missing entirely)
